...
const Name = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if ((name, userCountry)) {
      await db.collection('users').doc(currentUser.uid).set({ name, userCountry }, { merge: true });
        ...
        <div className="z-50 p-10 h-auto grid grid-cols-1 grid-rows-3 w-auto max-w-nameContainer w-full bg-white rounded-3xl">
          <div className="row-span-1 text-center">
            <div className="text-2xl font-medium">
              <img src="/gifs/sparkles.gif" height={50} width={50} className="inline-block relative bottom-2" /> What is your name?
            </div>
            <div className="text-base text-gray px-4">Oops, one more tiny step and we will get down to business.</div>
          </div>
          <div className="row-span-2">
            <form onSubmit={onSubmit} className="z-50 px-4 pt-2 rounded-2xl mt-4">
              <div className="mb-8">
              ...
              <div className="mb-8">
                <label className="block text-sm font-medium mb-2" htmlFor="userCountry">
                  Country
                </label>
                <CountryDropdown
                  id="country"
