...
export const createCheckoutSessionThreeDays = async (uid) => {
  const checkoutSessionRef = await db
    ...
    .add({
      ...
      success_url: `${window.location.origin}/dashboard/active-plan`,
      cancel_url: `${window.location.origin}/dashboard/active-plan`,
    });
...
export const createCheckoutSessionMonthly = async (uid) => {
  const checkoutSessionRef = await db
    ...
    .collection('checkout_sessions')
    .add({
      price: 'price_1IpW9BBdXPtKajNHAFlJNOPr', // Price key
      success_url: `${window.location.origin}/dashboard/active-plan`,
      cancel_url: `${window.location.origin}/dashboard/active-plan`,
    });
  // Wait for the CheckoutSession to get attached by the extension
...
export const createCheckoutSessionYearly = async (uid) => {
  const checkoutSessionRef = await db
    .collection('users')
    .doc(uid) // TO JEST DO ZMIANY Z PODSTAWOWEJ TEMPLATKI
    .collection('checkout_sessions')
    .add({
      price: 'price_1IpWCGBdXPtKajNHIY3ya4F3', // Price key
      success_url: `${window.location.origin}/dashboard/active-plan`,
      cancel_url: `${window.location.origin}/dashboard/active-plan`,
    });
  // Wait for the CheckoutSession to get attached by the extension
...
export const goToBillingPortal = async () => {
  ...
  const { data } = await functionRef({
    returnUrl: `${window.location.origin}/dashboard/active-plan`,
  });
